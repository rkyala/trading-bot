#!/usr/bin/env python3
"""
v3.8 PRODUCTION BOT: Mean-Reversion + Balanced Volatility + MCP
MCP authenticates automatically via browser
"""
...
class PositionManager:
    ...
    def place_order(self, signal: Dict) -> bool:
        try:
            ...
            logger.info(f"SIGNAL: {signal['symbol']} BUY {quantity} @ ${signal['price']:.2f} | Trailing Stop: ${position['trailing_stop']:.2f}")
    ...
    def close_position(self, exit_signal: Dict, circuit: CircuitBreaker) -> bool:
        try:
            ...
            logger.info(f"CLOSED: {exit_signal['symbol']} @ ${exit_signal['exit_price']:.2f} | P&L: ${pnl_usd:+.2f}")
...
class BotV38Balanced:
    ...
    def run_cycle(self):
        ...
        if self.circuit.check_halt_conditions():
            logger.warning("HALTED by circuit breaker")
            return

        position_mult, vol_status = self.volatility.get_position_multiplier()
        logger.info(f"Volatility: {vol_status} → Multiplier: {position_mult:.1f}x")
        ...
        logger.info(f"SCANNED {len(Config.SYMBOL_LIST)} symbols | Found {signals_found} signals")
        ...
        logger.info(f"COMPLETE | Open positions: {len(self.positions.positions)}")

if __name__ == "__main__":
    logger.info("="*100)
    logger.info("v3.8 BOT STARTED (LOCAL - No MCP)")
    logger.info("="*100)
