#!/usr/bin/env python3
"""
Schwab Block Trade Detector - Level 1 Quote Based (Workaround)
Uses bid/ask size changes to infer institutional block trades
Since schwab-py doesn't expose Time & Sales, we monitor Level 1 quotes for large size changes
"""
...
BLOCK_TRADE_CONFIG = {
    "min_shares": 10000,           # Minimum shares to trigger detection
    "min_notional_value": 200000,  # Minimum dollar value ($200K+)
    "size_surge_threshold": 5.0,   # Bid/ask size must increase 5x to trigger
    "watchlist": [
...
# ============================================================================
# BLOCK TRADE MONITOR (LEVEL 1 BASED)
# ============================================================================
...
class SchwabBlockTradeMonitor:
    ...
    def __init__(self):
        ...
        self.quote_history = defaultdict(lambda: {"bid_size": 0, "ask_size": 0, "last_price": 0})
    ...
    def _handle_quote_message(self, message: dict):
        """Process Level 1 quote data to detect bid/ask size changes (blocks)"""
        try:
            ...
            for tick in content:
                ...
                # Level 1 Quote field mapping:
                # "1": Bid Size, "2": Bid Price, "3": Ask Price, "4": Ask Size, "7": Last Price
                try:
                    bid_size = int(tick.get("1", 0)) * 100  # Bid size in contracts, convert to shares
                    ask_size = int(tick.get("4", 0)) * 100  # Ask size in contracts, convert to shares
                    last_price = float(tick.get("7", 0))
                ...
                # Detect size surge (bid or ask)
                detected_block = False
                block_type = ""
                detected_size = 0

                # Check bid size surge (large accumulation)
                if prev_bid_size > 0 and bid_size > 0:
                    bid_surge = bid_size / prev_bid_size
                    if bid_surge >= self.size_surge_threshold and bid_size >= self.min_shares:
                        detected_block = True
                        block_type = "Bid Accumulation"
                        detected_size = bid_size

                # Check ask size surge (large distribution)
                if prev_ask_size > 0 and ask_size > 0:
                    ask_surge = ask_size / prev_ask_size
                    if ask_surge >= self.size_surge_threshold and ask_size >= self.min_shares:
                        detected_block = True
    ...
    async def run(self):
        ...
        if now.hour < 9 or (now.hour >= 16 and now.minute >= 5):
            ...
            logger.info("ℹ️  Streaming paused during after-hours. Exiting.")
            return
        ...
        try:
            ...
            logger.info("ℹ️  Verify streaming is enabled on your Schwab account")
            return
        ...
        logger.info("=" * 80)
        logger.info("🟢 BLOCK TRADE DETECTOR ACTIVE (Level 1 Mode)")
        ...
        logger.info(f"   Method: Bid/Ask Size Change Detection")
        logger.info("=" * 80)
...
def main():
    logger.info("\n" + "=" * 80)
    logger.info("SCHWAB BLOCK TRADE DETECTOR - LEVEL 1 QUOTE MODE")
    logger.info("=" * 80)
