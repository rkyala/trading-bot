#!/bin/bash
# FinRL Daily Retraining: Runs every day at 20:00 UTC
# Auto-deploys new model if Sharpe >= 1.8 (prevents overfitting)

LOG_FILE="logs/finrl_daily_$(date +%Y%m%d_%H%M%S).log"
mkdir -p logs

echo "=== FinRL Daily Retraining ===" | tee -a "$LOG_FILE"
echo "Day: $(date +%A)" | tee -a "$LOG_FILE"
echo "Start: $(date)" | tee -a "$LOG_FILE"

# Train new model on past 60 days (daily instead of 365)
python3 finrl_working_trainer.py --days 60 2>&1 | tee -a "$LOG_FILE"
...
if (( $(echo "$SHARPE >= 1.8" | bc -l) )); then
    ...
    git commit -m "Auto: Daily FinRL retrain ($(date +%A), Sharpe=$SHARPE, 60-day window)" 2>&1 | tee -a "$LOG_FILE"
