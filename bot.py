...
Claude runs the full trading loop — stock selection, analysis, signal generation, trade execution.
"""
...
def run_trading_loop():
    ...
    system = f"""You are an autonomous stock trader with full control over a Robinhood brokerage account.
...
Each run you must:
1. Call get_trending_stocks to discover candidates.
2. For each promising candidate call get_news + fetch_market_data.
3. Use the Robinhood MCP tools (get_equity_positions, get_equity_quotes) to check current holdings and prices.
4. Make concrete BUY / SELL / HOLD decisions backed by technical signals and news sentiment.
5. For every trade: call review_equity_order first, then place_equity_order to execute.
6. Only trade stocks with market cap > $500M. Never exceed ${MAX_POSITION} per position.
7. Think out loud — log your reasoning for every decision.

Be decisive. Real money is on the line — but so is real opportunity. Manage risk, act on clear signals."""
