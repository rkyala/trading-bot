...
def fetch_market_data(symbol: str) -> dict | None:
    try:
        ...
        if hist.empty or len(hist) < 30:
            log.warning("%s: insufficient history", symbol)
            return None
...
def scan_symbol(symbol: str) -> None:
    ...
    log.info(
        "%s  $%.2f  RSI=%.1f  MACD=%+.3f  VWAP=$%.2f  EMA9=$%.2f  EMA20=$%.2f  vol=%.1fx",
...
def scan_all() -> None:
    ...
    for sym in STOCKS:
        scan_symbol(sym)
    log.info("session trades=%d  P&L=%+.2f", session_trades, session_pnl)
