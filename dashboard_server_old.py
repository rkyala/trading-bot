#!/usr/bin/env python3
"""Trading Bot Dashboard - Show Last & Next Cycle"""
...
def get_next_cycle_time():
    """Calculate next scheduled cycle time based on crontab"""
    now = datetime.now(CDT)
    
    # Cron schedule: 30 min intervals from 9:30 AM (14:30 UTC) to 3:00 PM (20:00 UTC) CDT, Mon-Fri
    cron_times = [
        (9, 30), (10, 0), (10, 30), (11, 0), (11, 30), (12, 0),
        (12, 30), (13, 0), (13, 30), (14, 0), (14, 30), (15, 0),
        (15, 30), (16, 0)  # 4:00 PM is last one
    ]
    ...
    for h, m in cron_times:
        cycle_time = now.replace(hour=h, minute=m, second=0, microsecond=0)
        if cycle_time > now:
            return cycle_time
    
    # After market hours, next is tomorrow 9:30 AM
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-bar { display: flex; justify-content: center; gap: 20px; flex-wrap: wrap; margin-top: 15px; }
        ...
        .cycle-time { font-size: 1.1em; color: #10b981; margin-bottom: 15px; }
        ...
        .footer { text-align: center; color: #94a3b8; margin-top: 30px; font-size: 0.9em; }
    </style>
</head>
<body>
    <div class="container">
        <header>
            ...
            <div class="status-bar">
                ...
                <div class="status-item"><span>Status:</span><span id="bot-status" style="font-weight:bold;color:#10b981">Ready</span></div>
            </div>
        </header>
        ...
        <div class="footer">Auto-refreshing every 2 seconds | Last updated: <span id="last-updated">--</span></div>
    </div>
        ...
        async function updateAll() {
            try {
                ...
                document.getElementById('stats-container').innerHTML = `
                    <div class="stat-row"><span class="stat-label">Total Cycles:</span><span class="stat-value">${stats.cycles}</span></div>
                    <div class="stat-row"><span class="stat-label">Total Signals:</span><span class="stat-value">${stats.signals}</span></div>
                    <div class="stat-row"><span class="stat-label">Analyzed:</span><span class="stat-value">${stats.analyzed}</span></div>
                    <div class="stat-row"><span class="stat-label">Skipped:</span><span class="stat-value">${stats.skipped}</span></div>
                `;

                document.getElementById('status-container').innerHTML = `
                    <div class="stat-row"><span class="stat-label">MCP:</span><span class="stat-value">${status.mcp_running ? '✅' : '⏸️'}</span></div>
                    <div class="stat-row"><span class="stat-label">Bot:</span><span class="stat-value">${status.bot_running ? '✅' : '⏸️'}</span></div>
                    <div class="stat-row"><span class="stat-label">Log Size:</span><span class="stat-value">${status.log_size}</span></div>
                `;

                document.getElementById('schedule-container').innerHTML = `
                    <div class="stat-row"><span class="stat-label">Frequency:</span><span class="stat-value">Every 30 min</span></div>
                    <div class="stat-row"><span class="stat-label">Hours:</span><span class="stat-value">9:30 AM - 4:00 PM CDT</span></div>
                    <div class="stat-row"><span class="stat-label">Days:</span><span class="stat-value">Mon - Fri</span></div>
                `;
                ...
                if (cycles.last_cycle_data) {
                    ...
                    if (d.analyzed && d.analyzed.length > 0) {
                        ...
                        if (d.analyzed.length > 20) lastHtml += '<div style="padding:6px;text-align:center;color:#94a3b8;font-size:0.7em;">+' + (d.analyzed.length - 20) + '</div>';
                        lastHtml += '</div></div>';
                    }
                    ...
                    if (!d.signals || d.signals.length === 0) {
                        lastHtml += '<div style="color:#94a3b8;padding:10px;background:#0f172a;border-radius:6px;font-size:0.9em;">No signals (market conditions not met)</div>';
                    }
                } else {
                ...
                let nextHtml = '<div style="color:#94a3b8;line-height:1.6;font-size:0.95em;">';
                nextHtml += '⏳ Waiting for next scheduled cycle...<br><br>';
                nextHtml += '<strong>What will happen:</strong><br>';
                nextHtml += '✅ Fetch 50 trending symbols<br>';
                nextHtml += '✅ Filter by $10B+ market cap<br>';
                nextHtml += '✅ Calculate ADX & Stochastic<br>';
                nextHtml += '✅ Check Fibonacci levels<br>';
                nextHtml += '✅ Generate signals (if any)<br>';
                nextHtml += '✅ Log all results<br>';
                nextHtml += '</div>';
...
if __name__ == "__main__":
    ...
    print("   Shows: Last Cycle + Next Cycle (scheduled time)")
